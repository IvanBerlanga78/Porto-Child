...
class WC_Widget_Price_Filter_Custom extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'CUSTOM Display a slider to filter products in your store by price.', 'woocommerce' );
		$this->widget_id          = 'woocommerce_price_filter';
		$this->widget_name        = __( 'Filter Products by Price Custom', 'woocommerce' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'std'   => __( 'Filter by price Custom', 'woocommerce' ),
	...
	protected function get_filtered_price() {
		...
		$featured = new WC_Product_Query( array(
		'featured' => true,
		'return' => 'ids',

		 ) );

		 $args = $featured->get_products();


		//$args = wc_get_products(array('featured'=>true));
		...
		/* WHERE {$wpdb->posts}.meta_key='_featured' AND {$wpdb->posts}.meta_value='yes' AND*/
