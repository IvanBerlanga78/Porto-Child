...
if ( 'builder' == $porto_product_layout ) :
	if ( isset( $porto_settings['product-single-content-builder'] ) && $porto_settings['product-single-content-builder'] ) {
		...
		$post_id = $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE post_type = 'product_layout' AND post_name = %s", $porto_settings['product-single-content-builder'] ) );
		...
		if ( function_exists( 'icl_object_id' ) ) {
			$lang_id = icl_object_id( $post_id, 'product_layout', false, ICL_LANGUAGE_CODE );
			if ( $lang_id ) {
				$post_id = $lang_id;
			}
		}
		...
		if ( $post_id ) {
			...
			if ( $shortcodes_custom_css ) {
				echo '<style>';
				echo wp_strip_all_tags( preg_replace( '#<style[^>]*>(.*)</style>#is', '$1', $shortcodes_custom_css ) );
				echo '</style>';
			}

			if ( get_post_meta( $post_id, 'container', true ) == 'yes' ) {
			...
			if ( $shortcodes_custom_js ) {
				echo '<script>';
				echo trim( preg_replace( '#<script[^>]*>(.*)</script>#is', '$1', $shortcodes_custom_js ) );
				echo '</script>';
			}
		}
...
<?php if ( $skeleton_lazyload ) : ?>
	...
	if ( ! isset( $summary_before_classes ) ) {
		$summary_before_classes = array( 'summary-before', 'col-md-6' );
		$summary_classes        = array( 'summary', 'entry-summary', 'col-md-6' );
	}
